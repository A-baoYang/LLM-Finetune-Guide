...
# pretrained_model=hfl/llama-3-chinese-8b-instruct
# pretrained_model=/workspace/Code/models/huggingface/llama-3-chinese-8b
model_name=llama2_7b
cache_dir=/workspace/Code/models/huggingface
dataset_tag=dcard-wom-zhcn
...
max_seq_length=2048
checkpoint_step=10
output_dir=/workspace/Code/LLM-Finetune-Guide/efficient-finetune/Chinese-LLaMA-2/lora/finetuned/sft-${dataset_tag}-${model_name}-lora-${model_name}-lora-${lora_rank}-${lora_alpha}-${lora_dropout}-${lr}-${per_device_train_batch_size}-${max_seq_length}-${num_train_epochs}-${gradient_accumulation_steps}-aaaa
...
CUDA_VISIBLE_DEVICES=1 torchrun --nnodes 1 --nproc_per_node 1 --master_port=29052 run_clm_sft_with_peft.py \
